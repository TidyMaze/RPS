...
class RPSServiceImpl extends RPSService {
  ...
  override def playTurn(in: PlayTurnRequest): Future[PlayTurnResponse] = {
    ...
    val (picked, updatedPlayerScore, updatedAiScore) = RPSEngine.buildAndPredict(state, windowSize, 3, humanRps).unsafeRunSync()
    ...
    games.put(gameId, state.copy(game = (state.game :+ humanRps).takeRight(20), updatedPlayerScore, updatedAiScore))
